...
const managementConfig = {
    ...
    attendance_methods: {
        ...
        columns: ['#', 'Method Name', 'Method Code', 'Description', 'Status', 'Created At', 'Actions'],
        formFields: `
            <div class="form-group">
                <label for="name">Method Name *</label>
                <input type="text" id="name" name="name" placeholder="e.g., QR Code, NFC Tap" required>
            </div>
            <div class="form-group">
                <label for="code">Method Code *</label>
                <input type="text" id="code" name="code" placeholder="e.g., qr_code, nfc, location" required>
                <small class="form-text text-muted">Use lowercase with underscores (e.g., qr_code, numeric_code)</small>
            </div>
            <div class="form-group">
                <label for="description">Description</label>
                <textarea id="description" name="description" rows="3" placeholder="Brief description of how this method works"></textarea>
            </div>
            <div class="form-group">
    ...
    statuses: {
        ...
        formFields: `
            <div class="form-group">
                ...
                <input type="text" id="name" name="name" placeholder="e.g., Active, Pending, Completed" required>
            </div>
            <div class="form-group">
...
// Updated the DOMContentLoaded event listener for initializing the page
...
function showItemDetails(type, item) {
    ...
    switch (type) {
        ...
        case 'attendance_methods':
            htmlContent += `
                <tr><th>Name</th><td>${item.name || 'N/A'}</td></tr>
                <tr><th>Code</th><td>${item.code || 'N/A'}</td></tr>
                <tr><th>Description</th><td>${item.description	 || 'N/A'}</td></tr>
                <tr><th>Name</th><td>${item.is_active || 'N/A'}</td></tr>
...
document.querySelector('.close-modal').addEventListener('click', () => {    
    try {
        ...
        console.error('Something went wrong here - unable to setup modal close button');
    }
});
...
async function loadItemData(type, id) {
    try {
        ...
        if (data.success) {
            ...
            handleApiError(data, 'load item data');
        }
    } catch (error) {
    ...
    tbody.innerHTML = filteredData.slice(startIndex, endIndex).map((item, index) => {
        ...
        switch (type) {
            ...
            case 'attendance_methods':
                row += `
                    <td>${truncateString(item.name || '',5)}</td>                   
                    <td>${truncateString(item.code || '',5)}</td>                   
                    <td>${truncateString(item.description || '',5)}</td>                   
                    <td>${truncateString(item.is_active || '',5)}</td>                   
                    <td>${truncateString(item.created_at ? formatDate(item.created_at) : '',5)}</td>
                `;
                break;
